...
async function check() {
  if (fs.existsSync(configPath)) {
    ...
    let qq, pwd, cookie, gachaDayNum, masterQQ;
    console.log("请按提示输入，生成配置文件config.js");
    console.log("需要重新设置的请删除config/config.js文件重来");
    qq = await question("请输入机器人QQ号：\n");
    if (!qq) {
      return await check();
    }
    ...
    gachaDayNum = await question("请输入每天抽卡次数【1-1000】，默认一次：");
