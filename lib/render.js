...
async function render(app = "", type = "", data = {}, imgType = "jpeg") {
  ...
  // 为 _res_path 设置默认值，兼容多种渲染情况
  if (typeof (data._res_path) === "undefined") {
    data._res_path = "../../../../resources/";
  }

  if (data._plugin) {
    data._app_res_path = `${_path}/plugins/${app}/resources`;
  } else {
    data._app_res_path = `${_path}/resources/${app}`;
  }
  ...
  if (!html[tplKey] || global.debugView) {
    ...
    html[tplKey] = fs.readFileSync(data._app_res_path + `/${type}/${type}.html`, "utf8");

    if (!fs.existsSync(_path + `/data/html/${app}/`)) {
      fs.mkdirSync(_path + `/data/html/${app}/`);
    }

    if (!fs.existsSync(_path + `/data/html/${app}/${type}/`)) {
      fs.mkdirSync(_path + `/data/html/${app}/${type}/`);
    }
  }
...
async function pluginRender(app = "", type = "", data = {}, imgType = "jpeg") {
  data._plugin = true;
  return await render(app, type, data, imgType);
}
...
export { render, browserInit, renderNum, pluginRender };
