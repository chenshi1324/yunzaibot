...
import * as MysApi from "./app/mysApi.js"
...
async function dealMsg(e) {
  ...
  if (e.isGroup && e.at != BotConfig.account.qq) {
    if (e.at == BotConfig.account.qq || /^\@喵喵bot/.test(e.msg)) {
  ...
  for (let val of command) {
    ...
    //原神查询功能时，@机器人代替#号
    if (val.type == "mysInfo" && e.at && e.at == BotConfig.account.qq && msg) {
      msg = "#" + msg.replace("#", "");
    }
    ...
    if (new RegExp(val.reg).test(msg) || val.reg == "noCheck") {
      ...
      try {
        let res = await YunzaiApps[type][name](e, { render, MysApi });
        if (res) {
          //设置cd
          setLimit(e);
          break;
