...
async function dealMsg(e) {
  ...
  for (let val of e.message) {
    switch (val.type) {
      ...
      case "image":
        ...
        e.img.push(val.url);
        redis.set(`cache:seq-img:${e.seq}`, val.url, { EX: 6000 });
        break;
      case "at":
        if (e.source && e.source.seq && !atImgSeq) {
          if (/\[图片\]/.test(e.source.message)) {
            atImgSeq = e.source.seq;
          }
  ...
  if (atImgSeq) {
    let imgUrl = await redis.get(`cache:seq-img:${atImgSeq}`);
    if (imgUrl) {
      e.img = [imgUrl];
  ...
  if (e.isGroup && e.at != BotConfig.account.qq) {
    if (e.at * 1 == BotConfig.account.qq * 1 || /^\@(喵喵bot|dev-bot)/.test(e.msg)) {
      e.msg = e.msg.replace('@喵喵bot', '').trim()
    } else if (e.isGroup && e.img && e.img.length > 0 && !e.msg) {
      // 处理圣遗物评分
    } else {
      return;
    }
  ...
  if (atQQ) {
    let atUid = await redis.get(`cache:qq-uid:${atQQ}`);
    let regUid = /[1|2|5][0-9]{8}/g.exec(atMsg);
    if (atUid) {
      e.msg += ` ${atUid}`;
    } else if (regUid && regUid[0]) {
      e.msg += ` ${regUid[0]}`
  ...
  // 设置会话的 用户、目标用户、请求用户
  e.selfUser = e.selfUser || await User.get(e.user_id, true);
  e.targetUser = e.targetUser || await User.getTargetUser(e, e.selfUser);
  e.reqUser = e.reqUser || await User.getReqUser(e, true);

  /*
    console.log(`selfUser: id:${e.selfUser.id}, uid:${e.selfUser.uid}`);
    console.log(`targetUser: id:${e.targetUser.id}, uid:${e.targetUser.uid}`);
    console.log(`reqUser: id:${e.reqUser.id}, uid:${e.reqUser.uid}`);
  */
  ...
  for (let val of command) {
    ...
    //原神查询功能时，@机器人代替#号
    if ((val.type == "mysInfo" || val.prehash) && e.at && e.at == BotConfig.account.qq && msg) {
      msg = "#" + msg.replace("#", "");
    }
    ...
    if (new RegExp(val.reg).test(msg) || val.reg == "noCheck") {
      ...
      try {
        ...
        let res = await YunzaiApps[type][name](e, { render: pluginRender, ...Components });
        if (res) {
          //设置cd
          setLimit(e);
          break;
