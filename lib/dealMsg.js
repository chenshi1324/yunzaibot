...
async function dealMsg(e) {
  ...
  a: for (let i in command) {
    let msg = e.msg;
    if (i == "mysInfo" && e.at && e.at == BotConfig.account.qq && msg) {
      e.msg = "#" + e.msg.replace("#", "");
    }

    //禁用功能
    if (e.groupConfig.disable && e.groupConfig.disable.length > 0) {
      if (e.groupConfig.disable.includes(i) || e.groupConfig.disable.includes("all")) {
        continue;
      }
    }
    if (typeof command[i] == "string") {
      ...
      if (reg.test(e.msg)) {
        ...
        setLimit(e);
        try {
          var res = await YunzaiApps[i][i](e);
        } catch (error) {
          Bot.logger.error(`${i}`);
      ...
      b: for (let j in command[i]) {
        ...
        if (command[i][j] == "noCheck") {
          ...
          try {
            var res = await YunzaiApps[i][j](e);
          } catch (error) {
            Bot.logger.error(`${i}.${j}`);
            Bot.logger.error(error);
          }
          if (res) {
            if (`${i}.${j}` != "face.add") {
...
function dealGroupRequest(e) {
  if (e.sub_type == 'invite'){
    ...
    e.approve(true);
    Bot.sendPrivateMsg(e.user_id,`已同意加群：${e.group_name}`)
  }
}
