...
export async function pushNewsTask(e = {}) {
    ...
    let maxNum = 1;
    //包含关键字不推送
    let reg = /冒险助力礼包|纪行|预下载|脚本外挂|集中反馈|作品展示|同人|已开奖|一图流|云·原神/g;
    ...
    let news2 = await mysBbs.getData({}, "getNewsList", { gids: 2, page_size: 10, type: 3 });
    let data = [
      { type: "公告", list: news.data.list },
      { type: "资讯", list: news2.data.list },
    ];
    ...
    for (let val of data) {
        for (let item of val.list) {
            ...
            if ((now - item.post.created_at <= interval && !pushed) || e.force) {
                pushNews.push(item);
                redis.set(key + item.post.post_id, "1", { EX: 3600 });
    ...
    for (let val of pushNews) {
        ...
        if (base64) pushData.unshift([`原神${val.type}推送：` , segment.image(`base64://${base64}`)]);
    }
