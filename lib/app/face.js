...
async function add(e) {
  ...
  if (e.groupConfig.imgAddLimit) {
    if (!Bot.gml.get(group_id) || !Bot.gml.get(group_id).get(e.user_id)) {
      e.reply(`请稍后再添加`);
      return;
    }
...
//根据关键词随机获取表情
async function getRandomImg(keyWord, page = 1,type = 1) {
  let url;
  switch (type) {
    case 1:
      url = `https://www.pkdoutula.com/search?keyword=${keyWord}&page=${page}`;
      break;
    case 2:
  ...
  let iamges = body.match(/data-original="(http:\/\/.*?)"/g);
...
export { add, addContext, del, get, list, listFuzzy, random, getRandomImg };
