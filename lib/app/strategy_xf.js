...
export async function strategy_xf(e) {
  ...
  if(structured_content && structured_content.length>0){
    for(let i in structured_content){
      if(typeof structured_content[i].insert == "string" && structured_content[i].insert.includes(name) && structured_content[i].insert.includes("图鉴")){
        img = image_list[structured_content[i-1].insert.image].url + "?x-oss-process=image/quality,q_80/auto-orient,0/interlace,1/format,jpg";
        break;
      }
    }
  } 
