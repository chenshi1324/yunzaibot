...
async function roleAll(e) {
  ...
  for (let i in res.avatars) {
    if (res.avatars[i].rarity >= 5) {
      rarity5++;
    }
    if (res.avatars[i].name != "旅行者" && res.avatars[i].actived_constellation_num >= 6) {
      constellation6++;
    }
  ...
  for (let val of res.world_explorations) {
    ...
    explor.push(
      { lable: val.name, num: `${val.exploration_percentage / 10}%` },
    );
  }
  line.push(explor)
  ...
  let base64 = await render("genshin", "roleAll", {
    ...
    line,
    avatars:res.avatars.slice(0,18),
    abyss,
  });
