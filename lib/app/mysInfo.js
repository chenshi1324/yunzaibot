...
async function roleAll(e) {
  ...
  if (avatars.length > 0) {
    //重新排序
    avatars = lodash.chain(avatars).orderBy(["sortLevel"], ["desc"]).slice(0, 18).orderBy(["sort"], ["desc"]).value();
  }
  //深渊
  ...
  if (list.length < 4) {
    for (let val of resAbyss.reveal_rank) {
      ...
      list.push({
        star: val.rarity,
        level: tmp.level || 80,
        name: tmp.name || roleIdToName(val.avatar_id, true),
        life: tmp.actived_constellation_num || 0,
      });
    }
  }
...
async function life(e) {
  ...
  let avatars = [];
  try {
    avatars = await getAllRoleDetail(e, uid, type);
  } catch (error) {
    checkRetcode(error, uid, e);
    return true;
  }
  ...
  list = lodash.sortBy(list, function (o) {
    return o.sort * -1;
  });
...
async function character(e) {
  ...
  if (e.msg == "#77") {
    ...
    roleId = roleIdToName(e.msg.replace(/#|\w/g, "").trim());
  }
  ...
  if (res.retcode == "-1") {
    limitSet(e);
    if (uidRes.isSelf) {
      let msg = lodash.truncate(e.sender.card, { length: 10 });
  ...
  let key = "";
  if (avatars.rarity >= 5) {
    key = `genshin:roleId5:${uid}`;
    redisId = await redis.get(key);
  } else {
    key = `genshin:roleId4:${uid}`;
    redisId = await redis.get(key);
  }
  if (redisId) {
...
async function wife(e) {
  ...
  if (!roleId5 || !roleId4) {
    ...
    allId = lodash.intersection(wifeData[i], allId);
    let tmpId = [];
    let res = [];
    let j = 0;
  ...
  if (list.length <= 0) {
    if (uidRes.isSelf) {
      let msg = lodash.truncate(e.sender.card, { length: 10 });
      let rand_face = [5, 9, 34, 35, 36, 37];
      rand_face = rand_face = rand_face[lodash.random(0, rand_face.length)];
      msg += `\n没有${e.msg}`;
      e.reply([msg, segment.face(rand_face)]);
      return true;
    } else {
...
async function getAllRoleDetail(e, uid, star = 5) {
  ...
  if (hasId) {
    ...
    let oldId = lodash.compact(hasId.id);
    ...
    if (hasId.id.length > 0) {
      ...
      redis.set(key, JSON.stringify(hasId), { EX: 2592000 });
    }

    //用redis的id查询
    oldId = lodash.chunk(oldId, 8);
    let res = [];
    for (let val of oldId) {
      res.push(
        mysApi(e, uid, "character", {
          character_ids: val,
          role_id: uid,
...
async function mysApi(e, uid, type, data = {}) {
  if (config.mysCookies.length <= 0) {
    e.reply("请打开config.js,配置米游社cookies");
    return false;
  }
  ...
  //获取uid集合
  let uid_arr = redis.get(`genshin:ds:qq:${e.user_id}`);
  let index = redis.get(`genshin:ds:uid:${uid}`);

  [uid_arr, index] = await Promise.all([uid_arr, index]);
  ...
  if (isNew) {
    let sadd = redis.sendCommand(["sadd", `genshin:ds:index:${index}`, uid]);
    let expire = redis.expire(`genshin:ds:index:${index}`, dayEnd);
    let indexRes = redis.set(`genshin:ds:uid:${uid}`, index, { EX: dayEnd });
    await Promise.all([sadd, expire, indexRes]);
  }
