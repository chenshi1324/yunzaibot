...
import { roleId, wifeData, roleId5, roleId4, actWeapon } from "../../config/genshin/roleId.js";
...
async function roleAll(e) {
  ...
  logger.info(`#角色2：${uid}获取数据中。。`);
  
  let role5 = await getAllRoleDetail(e, uid, 5);
  let role4 = await getAllRoleDetail(e, uid, 4);
  ...
  for (let i in avatars) {
    ...
    roleArr[avatars[i].id] = avatars[i];
  }
  ...
  //重新排序
  avatars = roleArr.sort(function (a, b) {
  ...
  //深渊
  let abyss = await abyssAll(e, uid, roleArr);
...
async function abyssAll(e, uid, roleArr) {
  let abyss = {};
  let res = await mysApi(e, uid, "spiralAbyss", { schedule_type: 1 });
  if (checkRetcode(res, uid, e)) {
    return abyss;
  } else {
    res = res.data;
  ...
  if (res.total_battle_times <= 0) {
    res = await mysApi(e, uid, "spiralAbyss", { schedule_type: 2 });
    if (checkRetcode(res, uid, e)) {
      return abyss;
    } else {
      res = res.data;
  ...
  let avatar = new Map();
  for (let val of roleArr) {
    if (val && val.id) {
      avatar.set(val.id, val);
    }
  }

  for (let val of tmpRole) {
    let tmp = avatar.get(val);
    list.push({
      star: tmp.rarity,
      level: tmp.level,
      name: tmp.name,
  ...
  if (list.length < 4) {
    for (let val of res.reveal_rank) {
      ...
      let tmp = avatar.get(val.avatar_id);
      list.push({
        star: val.rarity,
        level: tmp.level,
        name: tmp.name,
...
async function weapon(e) {
  ...
  weapon = weapon.sort(function (a, b) {
    return b.firstSort - a.firstSort;
  });
...
async function getAllRoleDetail(e, uid, star = 5) {
  ...
  if (hasId) {
    ...
    //接口查询
    if (new Date().getTime() - hasId.time > 3600 * 24 * 1000) {
      ids = ids.filter((v) => !hasId.id.includes(v));
      ...
      for (let val of ids) {
        ...
        if (res.retcode == 0) {
          hasId.id.push(val);
          roleDetail.push(res.data.avatars[0]);
        }
      }
      hasId.time = new Date().getTime();
      //保存redis
      await redis.set(key, JSON.stringify(hasId),{
        EX: 3600*24*30,
      });
    }
    //用redis的id查询
      ...
      for (let val of hasId.id) {
        ...
        if (tmpId.length >= 8 || i == hasId.id.length) {
          ...
          tmpId = [];
          if (res.retcode == 0) {
            roleDetail.push(...res.data.avatars);
          }
        }
      }
    }
