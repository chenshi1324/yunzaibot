...
export async function talentList(e) {
  ...
  if (hasCache && !/force/.test(e.msg)) {
    ...
    for (let idx in avatarData) {
      ...
      let avatar = lodash.pick(curr, "name,level,rarity,fetter,weapon".split(","));
      ...
      let skillRet = await getSkill(e, uid, curr);
    ...
    // 2小时缓存
    await redis.set(`cache:uid-talent:${uid}`, uid, { EX: 3600 * 2 });
  }
