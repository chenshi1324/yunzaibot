...
async function roleAll(e) {
  ...
  for (let i in res.avatars) {
    let rarity = res.avatars[i].rarity;
    let actived_constellation_num = res.avatars[i].actived_constellation_num;
    let level = res.avatars[i].level;
    let id = res.avatars[i].id - 10000000;

    if (rarity >= 5) {
      rarity = 5;
      rarity5++;
    }
    if (res.avatars[i].name != "旅行者" && actived_constellation_num >= 6) {
      constellation6++;
    }

    if (res.avatars[i].id == 10000005) {
      res.avatars[i].name = "空";
      actived_constellation_num = 0;
      level = 0;
    } else if (res.avatars[i].id == 10000007) {
      res.avatars[i].name = "荧";
      actived_constellation_num = 0;
      level = 0;
    }

    //id倒序，最新出的角色拍前面
    res.avatars[i].sort = rarity * 100000 + actived_constellation_num * 10000 + level * 100 + id;

    roleArr[res.avatars[i].id] = res.avatars[i];
  }
  ...
  //重新排序
  let avatars = res.avatars.slice(0, 12);
...
async function life(e) {
  ...
  for (let val of res.avatars) {
    ...
    val.sort = (val.actived_constellation_num + 1) * 10000 + val.level * 100 + (val.id - 10000000);
