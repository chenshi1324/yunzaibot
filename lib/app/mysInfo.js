...
export const rule = {
  ...
  todayQuery: {
    reg: "^#(今日|今天)*(素材|材料|天赋)[角色]*[信息]*[ |0-9]*$",
    priority: 208,
    describe: "【#今日素材】查看今天可以刷什么角色的",
  },
  talentList: {
    reg: "^#*(我的)?(技能|天赋|武器|角色|练度)+(汇总|统计|列表)(force)?$",
    priority: 206,
...
export async function todayQuery(e) {
  ...
  //今天素材
  let nowElement = genshin.todayList[week % 3];

  let mainList = [];
  let count = 0;

  for (let data in nowElement) {
    let temp = {
      name: "",
      area: "",
      //区分武器和天赋类型
    ...
    //获取角色数组
    let element = nowElement[data][1];
    //注入名称
    temp.name = data;
    temp.area = nowElement[data][0];
    //注入类型
    temp.isTalent = nowElement[data][2];
    ...
    if (temp.isTalent == 1) {
      ...
      temp.list = lodash.chain(temp.list).orderBy(["firstSort"], ["desc"]).slice(0, 21).orderBy(["sort"], ["desc"]).value();
    }
...
export async function talentList(e) {
  ...
  //   e.reply(`尚未配置，无法查询技能列表\n配置教程：https://docs.qq.com/doc/DUWNVQVFTU3liTVlO`);
  //   return true;
  // }
  ...
  let hasCache = await redis.get(`cache:uid-talent:${uid}`);
  if (hasCache && !/force/.test(e.msg)) {
    ...
    // 根据每日素材构建 角色->素材的映射关系
    let charTalentMap = {};
    genshin.todayList.forEach((weekCfg, week) => {

      lodash.forIn(weekCfg, (talentCfg, talentName) => {
        talentCfg[1].forEach((charName) => {
          charTalentMap[charName] = { name: talentName, week: week + 1 };
        })
      })
    })   
    ...
    for (let idx in avatarData) {
      ...
      let avatar = lodash.pick(curr, "id,name,level,rarity,fetter,weapon".split(","));
      ...
      lodash.forIn(talentConsCfg, (consLevel, key) => {
        // 天赋等级
        avatar[key] = skillRet[key] ? skillRet[key].level_current : '1';
        // 是否有命座加成
        avatar[`${key}_plus`] = avatar.cons * 1 >= talentConsCfg[key];
        // 真实天赋等级
        let realLv = key == "a" ? avatar[key] : (avatar[key] - (avatar[`${key}_plus`] ? 3 : 0));
        // 天赋书星级
        avatar[`${key}_lvl`] = talentLvMap[realLv];
    ...
    // 角色排序
    const getSortMark = function (a) {
      return (a.a + a.e + a.q) * 100000 +
        a.level * 999 +
        a.fetter * 99 +
        a.star * 9 +
        a.talentWeek;
    ...
    // 按技能、等级、好感、星级、周本顺序排序
    avatarRet.sort(function (a, b) {
      return getSortMark(b) - getSortMark(a);
    });
    fs.writeFileSync(cachePath + `${uid}.json`, JSON.stringify(avatarRet));
    //缓存
    await redis.set(`cache:uid-talent:${uid}`, uid, { EX: 3600 * cacheCd });
    delete skillLoading[e.user_id];
  }
