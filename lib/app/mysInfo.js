...
export async function roleAll(e) {
  ...
  resIndex.world_explorations = lodash.orderBy(res.world_explorations, ["id"], ["desc"]);
...
export async function role(e) {
  ...
  for (let val of res.world_explorations) {
    ...
    if(val.id == 6){
      let underground = lodash.find(res.world_explorations, function(o) { return o.id == 7; });
      if(underground){
        tmp.line.push({
...
export async function checkCookie(e) {
  ...
  for (let i in BotConfig.mysCookies) {
    ...
    if (res.retcode == 10001) {
      ...
      // 删除MysUser缓存中对应记录
      await MysUser.delBotCookie(ltuid);
      continue;
    }
    if (res.retcode != 0) {
      hasErr = true;
