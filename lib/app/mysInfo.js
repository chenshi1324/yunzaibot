...
const { roleId, wifeData } = require("../../config/genshin/roleId");
...
async function wife(e) {
  ...
  if (["老婆", "媳妇", "妻子", "娘子", "女朋友", "女友"].includes(e.msg)) {
    ...
    ["老公", "丈夫", "夫君", "郎君", "男朋友", "男友"].includes(e.msg)
  ) {
    i = 1;
  ...
  for (let val of res) {
    ...
    val.sort =
      val.level +
      val.fetter * 10 +
      val.actived_constellation_num * 5 * (val.rarity - 3) +
      (val.rarity - 4) * 20;
...
async function mysApi(e, uid, type, data = {}) {
  ...
  const res = await got(url, param).json();
...
module.exports = { role, abyss, life, weapon, wife, character };
