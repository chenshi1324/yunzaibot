...
function get_character(avatars) {
  ...
  //皮肤图片
  if (["芭芭拉", "琴", "刻晴", "凝光"].includes(avatars.name)) {
...
async function dailyNote(e) {
  let cookie, uid;
  let redisCookie = (e.msg = await redis.get(`genshin:cookie:${e.user_id}`));
  if (redisCookie) {
  ...
  if (res.data.resin_recovery_time > 0) {
    resinMaxTime = new Date().getTime() + res.data.resin_recovery_time * 1000;
  ...
  if (res.data.expeditions && res.data.expeditions.length >= 1) {
    remained_time = lodash.map(res.data.expeditions, "remained_time");
...
async function bingCookie(e) {
  ...
  if (reg.test(e.msg) && cookieContext[e.user_id]) {
  ...
  if (e.msg.includes("cookie_token")) {
    let headers = {
      Cookie: cookie,
    };
...
export { roleAll, role, abyss, abyssFloor, life, weapon, wife, character, character2, pokeCharacter, dailyNote, bingCookie, delCookie };
