...
export const rule = {
  ...
  todayQuery: {
    reg: "^#(今日|今天|每日)*(素材|材料|天赋)[角色]*[信息]*[ |0-9]*$",
    priority: 208,
    describe: "【#今日素材】查看今天可以刷什么角色的",
  },
  talentList: {
    reg: "^#*(我的)?(技能|天赋|武器|角色|练度)+(汇总|统计|列表)(force|五|四|5|4|星)*$",
    priority: 206,
...
export async function talentList(e) {
  ...
  if(star>=4 && avatarRet.length>8){
    //俄洛伊rarity是105...
    avatarRet = avatarRet.filter(item => item.rarity>5?5:item.rarity == star);
  }
