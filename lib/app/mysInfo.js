...
async function mysApi(e, uid, type, data = {}) {
  ...
  if (res.retcode != 0 && ![10102, 1008].includes(res.retcode)) {
...
async function checkCookie() {
  ...
  let { url, headers } = getUrl("getAnnouncement", "");
  let hasErr;
  ...
  for (let i in BotConfig.mysCookies) {
    ...
    if (res.retcode == 10001) {
      hasErr = true;
      Bot.logger.error(`配置cookie已失效:第${Number(i) + 1}个`);
      continue;
    }
    if (res.retcode != 0) {
      hasErr = true;
      Bot.logger.error(`配置cookie报错:第${Number(i) + 1}个`);
      continue;
    }
  }
  if (!hasErr) {
    Bot.logger.mark(`校验配置cookie完成:全部正常`);
  }
}

export { roleAll, role, abyss, abyssFloor, life, weapon, wife, character, character2, pokeCharacter, checkCookie, roleCard };
