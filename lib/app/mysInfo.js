...
async function getUid(e) {
  ...
  // 从绑定的体力cookie中获取
  if (NoteCookie[e.user_id]) {
    return { isSelf: true, uid: NoteCookie[e.user_id].uid }
  }
...
// 获取本次查询的最佳查询cookie
async function getBestCookie(uid, qq) {
  // 当前QQ存在cookie记录
  if (NoteCookie[qq] && NoteCookie[qq].uid == uid) {
    // console.log("NoteCookie Uid same")
    return {
      cookie: NoteCookie[qq].cookie,
      isSelf: true
    }
  }

  // 根据查询的UID查找
  let queryQQ = await redis.get(`genshin:uid-qq:${uid}`);
  if (queryQQ && NoteCookie[queryQQ] && NoteCookie[queryQQ].uid == uid) {
    // console.log("使用查询的UID换算QQ对应Cookie")
    return {
      cookie: NoteCookie[queryQQ].cookie,
      isSelf: true
    }
  }

  // 使用最后一次查询过的cookie
  let lastCookie = await redis.get(`genshin:uid-cookie:${uid}`);
  if (lastCookie) {
  ...
  // 如果当前用户绑定了cookie，则使用自身cookie查询
  // todo：优化，计数。超出时使用机器人cookie
  if (NoteCookie[qq]) {
    return {
      cookie: NoteCookie[qq].cookie,
      isSelf: false
...
// 获取查询的UID及最佳查询Cookie
async function getUidCookie(e, action = '', noticeMsg = '') {
  let uidRet = await getUid(e);
  let uid = uidRet.uid;

  let ckRet = await getBestCookie(uid, e.user_id);
  e.cookie = ckRet.cookie;
  e.isSelf = ckRet.isSelf;
  return {
    uid: uid,
    cookie: ckRet.cookie,
    isSelf: ckRet.isSelf,
...
async function mysApi(e, uid, type, data = {}) {
  ...
  redis.set(`cache:uid-cookie:${uid}`, headers.Cookie, { EX: 3600 * 23 });
