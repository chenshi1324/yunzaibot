...
import { roleId, wifeData, roleId5, roleId4, actWeapon, abbr, costumes } from "../../config/genshin/roleId.js";
...
function checkRetcode(res, uid, e) {
  ...
  switch (res.retcode) {
    ...
    case -200:
      ...
      break;
    case -300:
      e.reply("尚未配置米游社cookie，无法查询原神角色信息\n私聊发送【配置cookie】进行设置");
      break;
    case -400:
...
async function checkCookie(e) {
  ...
  for (let i in BotConfig.mysCookies) {
    ...
    if (!cookie.includes("ltoken")) {
      Bot.logger.error(`配置cookie错误:第${i + 1}个`);
      continue;
    }
    ...
    if (res.retcode == 10001) {
      hasErr = true;
      let msg = `配置cookie已失效:第${Number(i) + 1}个，ltuid:${ltuid}`;
    ...
    if (res.retcode != 0) {
      hasErr = true;
      let msg = `配置cookie报错:第${Number(i) + 1}个，ltuid:${ltuid}`;
      Bot.logger.error(msg);
      e.reply(msg);
      continue;
    }
...
async function setCookie(e) {
  ...
  e.reply(`配置成功，已配置${BotConfig.mysCookies.length}个cookie`);
  return true;
}

export { roleAll, role, abyss, abyssFloor, life, weapon, wife, character, character2, pokeCharacter, checkCookie, roleCard, setCookie };
