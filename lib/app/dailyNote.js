...
import { MysUser } from "../components/Models.js";

const _path = process.cwd();

let helpDoc = "https://docs.qq.com/doc/DY1V3RU91Q2pwdHB2";
...
export async function bingCookie(e) {
  ...
  //保存redis
  NoteCookie[e.user_id] = { uid, cookie: ltoken, isPush: true, isSignPush: false, maxTime };
  saveJson();

  // 将当前用户注册至MysUser缓存
  await MysUser.addNote(NoteCookie[e.user_id]);

  redis.set(`genshin:uid:${e.user_id}`, uid, { EX: 2592000 });
  redis.del(`cache:uid-talent-new:${uid}`);//删除技能列表缓存

  e.reply(`体力cookie配置成功`);
  let msg = `【#体力】查询当前树脂\n【#五星】查询更多角色内容`;
  msg += `\n【#开启推送】推送默认开启，体力大于120时推送`;
  msg += `\n【#关闭推送】关闭体力推送`;
...
export async function sign(e) {
  ...
  if (signInfo.data && signInfo.data.is_sign) {
    if (e.isTask) {
      redis.set(`genshin:dailyNote:signed:${e.user_id}`, "1", { EX: 3600 });
    }
...
async function save_ledger(e, uid, cookie) {
  ...
  let year = Number(format("yyyy", new Date()));
  let NowMonth = Number(format("MM", new Date()));
  ...
  for (let month of monthArr) {
    ...
    if (NoteData[year][month] && NowMonth != month) continue;
