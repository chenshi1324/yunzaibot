...
async function bingCookie(e) {
  ...
  let param = cookie.match(/ltoken=([^;]+;)|ltuid=([^;]+;)|cookie_token=([^;]+;)/g);
...
async function openDailyNote(cookie) {
  ...
  let account_id = cookie.match(/ltuid=([^;]+;)/g)[0].replace(/ltuid=|;/g, "");
...
function resinhelp(e) {
  ...
  setTimeout(() => {
    ...
    e.reply(`代码：document.cookie.match(/ltoken([^;]+;){2}|cookie_token([^;]+;){2}/g).join('')`);
  }, 1000);
  return true;
}
