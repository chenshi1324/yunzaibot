...
export async function bingCookie(e) {
  ...
  Bot.logger.mark(`添加体力cookie成功:${e.user_id}`);
  ...
  let msg = `体力cookie配置成功\n【#体力】查询当前树脂\n【#五星】查询更多角色内容`;
  msg += `\n【#开启推送】推送默认开启，体力大于120时推送（需要加好友）`;
  msg += `\n【#关闭推送】关闭体力推送`;
...
export async function sign(e) {
  //获取cookie
  let cookie, uid;
  let key = `genshin:dailyNote:sign:${e.user_id}`;
  ...
  if (!cookie.includes("cookie_token")) {
    e.reply("配置cookie不完整，不支持米游社签到\n请发送完整cookie：ltoken=***;ltuid=***;cookie_token=***;");
    return true;
  }
  ...
  if (signInfo.retcode != 0) {
    ...
    if (signInfo.message == "尚未登录" && cookie.includes("cookie_token")) {
      ...
      Bot.logger.mark(`体力cookie错误:${JSON.stringify(signInfo)}`);
    }
    return true;
  }
  ...
  //获取签到奖励
  let reward;
  response = await bbs_sign_home(uid, cookie);
  if (response.ok) {
    let home = await response.json();
    if (home.data && home.data.awards[signInfo.data.total_sign_day - 1]) {
      reward = home.data.awards[signInfo.data.total_sign_day - 1];
  ...
  if (signInfo.data && signInfo.data.is_sign) {
    e.reply(`今日米游社已签到\n奖励：${reward}`);
    redis.set(key, reward, { EX: getDayEnd() });
    return true;
  }
  ...
  if (res.retcode == 0 || res.retcode == -5003) {
    ...
    Bot.logger.mark(`米游社签到[${e.user_id}]：${res.message}`);
  }
...
export async function signTask() {
  ...
  for (let [user_id, cookie] of Object.entries(NoteCookie)) {
    ...
    //今天已经签到
    let key = `genshin:dailyNote:sign:${user_id}`;
    let sgined = await redis.get(key);
    if (sgined) {
      continue;
    }
    ...
    e.reply = (msg) => {
      if (msg.includes("签到成功") && cookie.isSignPush == false) {
