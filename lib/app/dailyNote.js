...
export async function sign(e) {
  ...
  if (reward && reward[signInfo.data.total_sign_day - 1]) {
...
async function getReward(uid, cookie) {
  let month = format("MM", new Date());
  let key = `genshin:dailyNote:signReward:${month}`;
  ...
  if (home.retcode == 0 && home.data && home.data.awards && home.data.awards.length > 0) {
    ...
    e.reply = (msg) => {
      if (msg.includes("签到成功") && (cookie.isSignPush === true || cookie.isSignPush === undefined)) {
        ...
        Bot.pickUser(user_id).sendMsg(msg).catch((err) => {
          Bot.logger.mark(`签到推送失败:${user_id}`);
        });
      }
    };
