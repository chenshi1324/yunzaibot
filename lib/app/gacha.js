...
//每日抽卡次数
const dayNum = gachaConfig.dayNum;

let up4 = gachaConfig.up4;
let up5 = gachaConfig.up5;

//去除当前up的四星
role4 = role4.filter(function (val) {
  return !up4.includes(val);
});
...
async function gacha(e) {
  ...
  if (!gachaData) {
    ...
    if (new Date().getTime() >= gachaData.today.expire) {
      gachaData.today = { role: [], expire: end.dayEnd };
    }
    if (new Date().getTime() >= gachaData.week.week_end) {
      gachaData.week = { num: 0, expire: end.weekEnd };
    }
  }

  if (gachaData.today.num >= dayNum) {
    let msg = Substr(name, 13);
  ...
  if (res5.length > 0) {
    info = `累计「${res5[res5.length - 1].num}抽」`;
  }
  ...
  if (base64) {
    e.reply(`[CQ:image,file=base64://${base64},type=face]`);
  }
...
function getEnd() {
  ...
  if (now.getHours() < 4) {
    ...
    dayEnd =
      new Date(year, month, day, "23", "59", "59").getTime() + 3600 * 4 * 1000;
  }
