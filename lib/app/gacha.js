...
export const rule = {
  gacha: {
    reg: "^#*([1-9]+0|[武器池]*[一二两三四五六七八九十百]+|抽|单)[连抽卡奖][12武器池]*$",
    priority: 100,
  ...
  update: {
    reg: "^#*更新卡池$", //匹配的正则
    priority: 102, //优先级，越小优先度越高
    describe: "【更新卡池】更新卡池", //描述说明
  },
};
...
function getNowPool(upType) {
  ...
  upW4 = end.weapon4;
  upW5 = end.weapon5;
  poolName = abbr[up5[0]] ? abbr[up5[0]] : up5[0];
...
export async function weaponBing(e) {
  ...
  if (upW5.length <= 0) {
    upW5 = gachaConfig.pool[gachaConfig.pool.length - 1].weapon5;
  }
  ...
  if (gachaData.weapon.type >= 2) {
    ...
    msg = "\n定轨成功：" + upW5[gachaData.weapon.type - 1];
  }
...
export async function update(e) {
  ...
  let pool = getNowPool(1);
  let role = [...pool.raw_up5,...pool.up4];
  let weapon = [...pool.upW5,...pool.upW4];
  for(let name of role){
    let path =  `/resources/genshin/gacha/character/${name}.png`;
  ...
  for(let name of weapon){
    let path =  `/resources/genshin/gacha/weapon/${name}.png`;
    if(fs.existsSync(`.${path}`)){
      continue;
    }
    common.downImg(path);
