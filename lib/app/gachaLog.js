...
import { abbr, abbr2 } from "../../config/genshin/roleId.js";
...
async function bing(e) {
  ...
  if (res.retcode == -100) {
    e.reply("链接不完整，请长按全选复制全部内容，或者复制的不是历史记录页面链接");
    return true;
  }
  if (res.retcode != 0) {
    e.reply("链接复制错误");
...
async function getLog(e) {
  ...
  if (e.reply) {
    ...
    Bot.logger.mark(`更新${typeName}记录成功:${e.user_id}`);
  }
  if (base64) {
    //发送文件方式 但是不是好友
    if (e.reply) {
...
export async function history(e) {
  ...
  let name = lodash.truncate(e.sender.card, { length: 12 });
  e.reply([segment.at(e.user_id, name), segment.image(`base64://${base64}`)]);
...
function analyseHistory(all, e, type) {
  ...
  for (let i in pool) {
    ...
    res.push(pool[i]);
    line += Math.ceil(pool[i].role.length / 6);
    if (e.isGroup && line >= 19) {
      break;
    }
  }
