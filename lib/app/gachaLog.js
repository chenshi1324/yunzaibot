...
async function bing(e) {
  ...
  redis.set(key + e.user_id, params.authkey, { EX: 86400 });

  await e.reply("链接发送成功");
  e.isBing = true;
  e.msg = "角色记录";
  await getLog(e);
  e.msg = "常驻记录";
...
async function getLog(e) {
  let msg = e.msg.replace(/#|抽卡|记录|祈愿/, "");
  ...
  let base64 = await render("genshin", "gachaLog", {
    save_id: mainUid,
    uid: mainUid,
    allNum: data.allNum,
    line,
...
function analyse(all) {
  ...
  if (["莫娜", "七七", "迪卢克", "琴", "刻晴"].includes(fiveArr[0].name)) {
...
function helpPort(e) {
  let msg = e.msg.replace(/#|帮助/, "");

  e.reply(
    segment.image(`file://${_path}/resources/logHelp/记录帮助-${msg}.png`)
  );

  return true;
}
export { bing, getLog, help, helpPort };
