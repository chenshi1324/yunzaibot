...
export async function zupdate(e) {
    ...
    if (isRestart){
        command = "npm restart";
        e.reply("正在重启云崽，请稍等(注：仅支持npm start启动)");
    }else if(isForce){
    ...
    var ls = exec(command, function (error, stdout, stderr) {
        if (error) {
            ...
            if(isChanges){
                ...
                e.reply("失败！\nError code: "+error.code+"\n"+error.stack+"\n\n本地代码与远程代码存在冲突,上面报错信息中包含冲突文件名称及路径，请尝试处理冲突\n如果不想保存本地修改请使用【#强制更新】\n(注意：强制更新命令会忽略所有本地对云崽本身文件的修改，本地修改均不会保存，请注意备份，lib/example不影响)");
            }else if(isNetwork){
...
export async function checkupdate(e) {
    ...
    // console.log("redis",await redis.get("git hub pushed_at"));

    let oldtime = new Date(await redis.get("git hub pushed_at"));
...
export async function setUpdatetime(e) {
    ...
    // redis.set(`git hub pushed_at`, "2022-04-04T03:51:03Z");  test
    ...
    redis.set(`git hub pushed_at`, oldtimetemp);
