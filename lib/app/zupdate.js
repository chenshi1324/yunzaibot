...
export async function zupdate(e) {
  ...
  var ls = cp.exec(command, function (error, stdout, stderr) {
    if (error) {
...
export async function restartApp(e) {
  ...
  try {
    let res = cp.execSync(`npm run list`).toString();

    await redis.set("Yunzai:restart", data, { EX: 120 });

    if (res.includes("Yunzai")) {
      res = cp.execSync(`npm run restart`).toString();
    } else {
      res = cp.execSync(`npm run start`).toString();
    }
    process.exit();
  } catch (error) {
    e.reply(`操作失败！\n${error.stack}`);
