...
export async function scoreImg(e) {
  ...
  let OrcRes = await response.json();
  ...
  if (!response.ok) {
    ...
    if (OrcRes.code == "50002") {
      e.reply([segment.at(e.user_id), "请从背部选择圣遗物截图，图片要包含主词条和副词条"]);
    } else {
  ...
  if (OrcRes?.sub_item?.length <= 0) {
    e.reply([segment.at(e.user_id), "请从背部选择圣遗物截图"]);
    cancel(e);
    return;
  }
  ...
  if (response.ok) {
    ...
    sub_percent = rateRes?.sub_percent;
  }
