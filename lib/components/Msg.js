...
Msg.replyNeedCookie = function (e, replyMsg) {
  ...
  let helpMsg = `获取cookie后发送至当前聊天窗口即可，Cookie获取方式：${cookieDoc}`;
...
* 5. 当前用户名片包含 uid
* */
Msg.getTargetMysUser = async function (e, targetType) {

  if (e.targetUser && e.targetUser.uid) {
    return e.targetUser;
  }
  ...
  if (e.msg) {
    ...
    if (targetUid) {
      ...
      if (!mysUser) {
        console.log('getMysUser fail, uid', targetUid)
      }
      // 将查询记录绑定至当前用户
...
*
* */
Msg.initMysApi = async function (e, cfg) {
  let {
    targetType = "all",
    cookieType = "all",
  } = cfg;
  ...
  e.selfUser = selfUser;
  // 检查调用次数
  // if (!await checkLimitGet(e, selfUser)) return false;
...
Msg.getCookieUser = async function (e, cfg) {
  ...
  if (cookieType === "self") {
    ...
    if (!targetUser.cookie) {
      // todo 完善提示
      e.replyNeedCookie(e, `UID:${targetUser.uid}尚未绑定Cookie`);
      return false;
    }
    return targetUser;
  }
...
const checkLimitGet = async function (e) {
  return await limitGet(e);
};

// 检查权限
const checkAuth = async function (e, cfg) {
  let {
    auth = "all",
    actionName = "进行操作",
    replyMsg = "",
