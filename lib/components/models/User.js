...
class User extends BaseModel {
  ...
  refreshCache() {
    // 与QQ用户相关的缓存刷新逻辑均放置在此
    Cache.set("id-uid", this.id, this.uid);

    redis.set(`genshin:uid:${this.id}`, this.uid, { EX: 2592000 });
    redis.del(`cache:uid-talent-new:${this.uid}`);//删除技能列表缓存
  ...
  * */
  async bindMysUser(mysUser) {
    ...
    NoteCookie[this.id] = {
      ...
      isSignPush: !!target.isSignPush, // 默认为false
