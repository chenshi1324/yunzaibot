...
class User extends BaseModel {
  ...
  * */
  async getMysUser() {
    ...
    let uid = await Cache.get('id-uid', this.id);
    if (uid) {
  ...
  // 记录用户的请求记录
  async addQuery(uid) {
    await redis.sAdd(`${queryKey}:${this.uid}`, uid);
    await redis.expire(`${queryKey}:${this.uid}`,getDayEnd());
  }
  ...
  async checkLimit(limit = 15) {
    ...
    let queryList = await redis.sMembers(`${queryKey}:${this.uid}`) || [];
...
const getUser = async function (query, allowDraft = true) {
