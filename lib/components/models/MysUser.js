...
class MysUser extends BaseModel {
  ...
  async getCookie() {
    if (!this._cookie) {
      ...
      console.log('find cookie from cache', cookie)
      if (cookie) {
        this._cookie = cookie;
      }
    }
  ...
  * */
  ...
  _reclaim() {
    ...
    this._reclaimFn = setTimeout(() => {
      delete mysUserMap[this.uid];
      console.log(`UID:${this.uid} reclaimed `)
...
MysUser.init = async function (uids, updateStat = false) {
  ...
  for (let nid in NoteCookie) {
    console.log(nid, NoteCookie[nid]);
    let ckCfg = NoteCookie[nid];
    let mUser = await MysUser.get(ckCfg.uid * 1);
    if (mUser) {
      mUser.setCookie(ckCfg.cookie);
...
MysUser.get = async function (uid, cookie = '') {
  console.log('MysUser.get', uid, cookie);
  let mUser;
  if (mysUserMap[uid]) {
    console.log('from cache', uid, mysUserMap[uid].cookie)
    return mysUserMap[uid]._reclaim();
  }
...
MysUser.getUidByCookie = async function (cookie = '') {
  ...
  let uid = await redis.get(`ckidKey:${token.cookie_token}`);
  if (uid) {
    return uid;
  }
  ...
  if (!response.ok) {
    // e.reply("米游社接口错误");
    await redis.set(`ckidKey:${token.cookie_token}`, false, { EX: 3600 * 24 * 30 });
    return true;
  }
  ...
  uid = uid * 1 || false;
  await redis.set(`ckidKey:${token.cookie_token}`, uid, { EX: 3600 * 24 * 30 });
  return uid;
}
...
function getDayEnd() {
  ...
  console.log('dayEnd', dayEnd - parseInt(now.getTime() / 1000));
