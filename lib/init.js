...
async function init() {
  ...
  for (let val of readDir) {
    try {
      let tmp = await import(`./app/${val}`);
      if (!tmp.rule) continue;
      let type = val.replace(".js", "");
      YunzaiApps[type] = tmp
...
function firstLogin() {
  ...
  let msg = "欢迎使用Yunzai-Bot\n发送【#帮助】查看指令说明";

  Bot.sendPrivateMsg(BotConfig.masterQQ[0], msg).catch((err) => {
    Bot.logger.error(err);
  });

  if (BotConfig.mysCookies && BotConfig.mysCookies.length>0 && BotConfig.mysCookies[0]!='') {
    msg = `已配置${BotConfig.mysCookies.length}个cookie，配置更多cookie\n发送【配置cookie】进行设置\n例如：\n配置'ltoken=***;ltuid=***;'`;
  } else {
    msg = "目前尚未配置米游社cookie，无法查询原神角色信息\n发送【配置cookie】进行设置\n例如：\n配置'ltoken=***;ltuid=***;'";
  }
