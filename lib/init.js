...
async function init() {
  ...
  const client = createClient({
    url: `redis://:${BotConfig.redis.password}@${BotConfig.redis.host}:${BotConfig.redis.port}`,
  });
  ...
  let dir = {
    genshin: ["abyss", "abyssFloor", "character", "gacha", "life", "role","roleAll", "weapon", "gachaLog", "gachaJson"],
    face: ["list"],
  };
  for (let i in dir) {
